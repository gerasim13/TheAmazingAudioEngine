    ...
    for ( int i=channel->callbacks.count-1, filterIndex=0; i>=0; i-- ) {
        ...
        if ( callback->flags & kFilterFlag ) {
            if ( filterIndex == arg->nextFilterIndex ) {
                ...
                return ((AEAudioFilterCallback)callback->callback)((__bridge id)callback->userInfo,
                                                                   (__bridge AEAudioController *)channel->audioController,
                                                                   &channelAudioProducer,
                                                                   (void*)&filterArg,
                                                                   arg->ioActionFlags,
                                                                   &arg->timeStamp,
                                                                   *frames, audio);
...
static OSStatus inputAvailableCallback(void *inRefCon, AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inBusNumber, UInt32 inNumberFrames, AudioBufferList *ioData) {
    ...
    if ( !THIS->_outputEnabled ) {
        ...
        serviceAudioInput(THIS, ioActionFlags, NULL, inTimeStamp, THIS->_lastAvailableInputFrames);
    }
#else
...
static OSStatus topRenderNotifyCallback(void *inRefCon, AudioUnitRenderActionFlags *ioActionFlags, const AudioTimeStamp *inTimeStamp, UInt32 inBusNumber, UInt32 inNumberFrames, AudioBufferList *ioData) {
    ...
    if ( *ioActionFlags & kAudioUnitRenderAction_PreRender ) {
        ...
        if ( THIS->_inputEnabled ) {
            serviceAudioInput(THIS, ioActionFlags, inTimeStamp, &THIS->_lastInputBusTimeStamp, THIS->_lastAvailableInputFrames);
        }
#endif
...
static void serviceAudioInput(__unsafe_unretained AEAudioController * THIS,
                              AudioUnitRenderActionFlags *ioActionFlags,
                              const AudioTimeStamp *outputBusTimeStamp,
                              const AudioTimeStamp *inputBusTimeStamp,
    ...
    if ( result == noErr ) {
        ...
        for ( int tableIndex = 0; tableIndex < table->count; tableIndex++ ) {
            ...
            for ( int i=0; i<entry->callbacks.count && result == noErr; i++ ) {
                ...
                ((AEAudioReceiverCallback)callback->callback)((__bridge id)callback->userInfo, THIS, AEAudioSourceInput, ioActionFlags, &timestamp, inNumberFrames, audioBufferList);
            }
        }
...
- (id)initWithAudioDescription:(AudioStreamBasicDescription)audioDescription options:(AEAudioControllerOptions)options {
    ...
    BOOL enableMultiroute       = options & AEAudioControllerOptionMultiroutOutput;
    
#if TARGET_OS_IPHONE || TARGET_OS_MACCATALYST
    if ( enableMultiroute ) {
        _audioSessionCategory = AVAudioSessionCategoryMultiRoute;
    }
    else if ( enableInput ) {
        _audioSessionCategory = enableOutput ? AVAudioSessionCategoryPlayAndRecord : AVAudioSessionCategoryRecord;
    }
    else {
    ...
    _multirouteEnabled = enableMultiroute;
...
- (AEChannelGroupRef)createChannelGroupWithinChannelGroup:(AEChannelGroupRef)parentGroup completionBlock:(void (^)(AEChannelGroupRef))block {
    if ( atomic_load_explicit(&parentGroup->channelCount, memory_order_relaxed) == kMaximumChannelsPerGroup ) {
...
- (void)setVolume:(float)volume forChannelGroup:(AEChannelGroupRef)group {
    ...
    AudioUnitParameterValue value = volume;
    atomic_store_explicit(&group->channel->volume, volume, memory_order_release);
...
-(float)volumeForChannelGroup:(AEChannelGroupRef)group {
    return atomic_load_explicit(&group->channel->volume, memory_order_acquire);
}
...
- (void)setPan:(float)pan forChannelGroup:(AEChannelGroupRef)group {
    ...
    AudioUnitParameterValue value = pan;
    atomic_store_explicit(&group->channel->pan, pan, memory_order_release);
...
-(float)panForChannelGroup:(AEChannelGroupRef)group {
    return atomic_load_explicit(&group->channel->pan, memory_order_acquire);
}
...
-(void)setAudioSessionCategory:(NSString *)audioSessionCategory {
    ...
    if ( ![_audioSessionCategory isEqualToString:AVAudioSessionCategoryMultiRoute] ) {
...
-(void)setPreferredOutputNumberOfChannels:(int)preferredOutputNumberOfChannels
{
    ...
    _preferredOutputNumberOfChannels = audioSession.maximumOutputNumberOfChannels >= preferredOutputNumberOfChannels ? preferredOutputNumberOfChannels : (int)audioSession.preferredOutputNumberOfChannels;
    [self updateOutputDeviceStatus];
}
...
- (BOOL)initAudioSession {
    ...
    // Add observer
    [audioSession addObserver:self forKeyPath:NSStringFromSelector(@selector(outputNumberOfChannels)) options:NSKeyValueObservingOptionNew context:nil];
...
- (BOOL)updateOutputDeviceStatus {
    AVAudioSession *audioSession = [AVAudioSession sharedInstance];
    int numberOfOutputChannels = (int)audioSession.outputNumberOfChannels;
    if (_numberOfOutputChannels != numberOfOutputChannels) {
...
- (void)configureChannelsForGroup:(AEChannelGroupRef)group {
    ...
    for ( int i = 0; i < (group ? MAX(atomic_load_explicit(&group->channelCount, memory_order_relaxed), priorBusCount) : 1); i++ ) {
        ...
        if ( channel->type == kChannelTypeChannel ) {
            ...
            for ( int i=0; _multirouteEnabled && i<subgroup->channelCount; i++ ) {
...
static void handleCallbacksForChannel(AEChannelRef channel,
    ...
    for ( int i=0; i<channel->callbacks.count; i++ ) {
        ...
        if ( callback->flags & kReceiverFlag ) {
            ((AEAudioReceiverCallback)callback->callback)((__bridge id)callback->userInfo, (__bridge AEAudioController*)channel->audioController, channel->ptr, ioActionFlags, inTimeStamp, inNumberFrames, ioData);
        }
    }
}
...
static void performLevelMonitoring(audio_level_monitor_t* monitor, AudioBufferList *buffer, UInt32 numberFrames) {
    ...
    for ( int i=0; i<monitor->scratchBuffer->mNumberBuffers && i < kMaximumMonitoringChannels; i++ ) {
        ...
        if ( peak > monitor->chanPeak[i] )
        {
            monitor->chanPeak[i] = peak;
        }
        if ( peak > monitor->peak )
        {
            monitor->peak = peak;
        }
